...
def args_handler(context, content, pargs, kwargs):
    for key, value in sorted(kwargs.items()):
        if key == 'id':
            try:
                obj = Blog.objects.get(id=value)
                return obj.content.replace(" ", "")
            except Blog.DoesNotExist:
                print("Blog matching shortcode does not exist.")
    return '<p style="color:red">Error in shortcode.</p>'


def parse_shortcodes(data):
    parser = Parser()
    return parser.parse(data.content, context=None)
